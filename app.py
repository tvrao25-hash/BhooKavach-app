...
with ctrl_side:
    ...
    # In-Memory Search Loop matching dictionaries
    search_input = st.text_input("🔍 Filter Property Roster by Sub-Sector Keyword:", "", placeholder="e.g., Gachibowli, Thullur...")
    if search_input:
        matched_list = [p for p in market_feed if search_input.lower() in p["sector"].lower() or search_filter = search_input.lower() in p["name"].lower()]
    else:
